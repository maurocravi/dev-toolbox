...
const LINK_COLOR_MAP: Record<string, { bg: string; text: string; dot: string }> = {
  blue: { bg: "rgba(59, 130, 246, 0.12)", text: "#60a5fa", dot: "#3b82f6" },
...
interface LinksAccountsManagerProps {
  links: ProjectLink[];
  accounts: ProjectAccount[];
  onChange: (links: ProjectLink[], accounts: ProjectAccount[]) => void;
  saving?: boolean;
}

export default function LinksAccountsManager({
  links,
  accounts,
  onChange,
  saving = false,
  ...
  const [revealedPass, setRevealedPass] = useState<Set<number>>(new Set());
  ...
  const handleAddLink = () => {
    ...
    const newLink: ProjectLink = { label: trimmedLabel, url: trimmedUrl, color: formLinkColor };
    const newLinks =
      editingLinkIndex !== null
        ? links.map((l, i) => (i === editingLinkIndex ? newLink : l))
        : [...links, newLink];

    onChange(newLinks, accounts);
    setFormLinkLabel("");
    setFormUrl("");
              ...
              <div key={index} className="bg-[var(--card-bg)] border border-[var(--card-border)] rounded-xl p-3 transition-[border-color] duration-150 hover:border-[#2a2a32]">
                ...
                <div className="rounded-md px-2 py-1.5 space-y-1" style={{ backgroundColor: colors.bg }}>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <code className="text-[0.6875rem] font-mono text-[var(--foreground)] leading-snug">
                      {revealedPass.has(index) ? account.password : "••••••••"}
                    </code>
                    <button
                      className="ml-auto flex items-center justify-center w-5 h-5 rounded bg-transparent border-none text-zinc-500 cursor-pointer transition-all duration-150 hover:text-[var(--accent-hover)]"
                      onClick={() => toggleReveal(index)}
                      aria-label={revealedPass.has(index) ? "Ocultar contraseña" : "Mostrar contraseña"}
                      title={revealedPass.has(index) ? "Ocultar contraseña" : "Mostrar contraseña"}
                    >
                      {revealedPass.has(index) ? (
