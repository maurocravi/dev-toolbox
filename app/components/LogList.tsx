...
export default function LogList({
  ...
  const handleCopyTaskName = async (log: TimeLog) => {
    try {
      await navigator.clipboard.writeText(log.taskName);
      setCopiedId(log.id);
      setTimeout(() => setCopiedId(null), 1500);
    } catch {
      // Fallback for browsers that don't support clipboard API
      const textarea = document.createElement("textarea");
      textarea.value = log.taskName;
                        ...
                        <div className="flex items-center gap-2">
                          {isEditing ? (
                            ...
                            <>
                              ...
                              <button
                                onClick={() => handleCopyTaskName(log)}
                                className={`flex items-center justify-center w-7 h-7 rounded-lg bg-transparent border-none cursor-pointer transition-all duration-150 flex-shrink-0 disabled:opacity-40 disabled:cursor-not-allowed ${
                                  copiedId === log.id
                                    ? "text-emerald-400 hover:bg-[rgba(16,185,129,0.12)]"
                                    : "text-zinc-500 hover:bg-[rgba(99,102,241,0.1)] hover:text-[var(--accent-hover)]"
                                }`}
                                aria-label={`Copiar tarea ${log.taskName}`}
                                disabled={savingId !== null || togglingJiraId !== null}
                                title={copiedId === log.id ? "¡Copiado!" : "Copiar título"}
                              >
                                {copiedId === log.id ? (
                                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
                                    <polyline points="20 6 9 17 4 12" />
